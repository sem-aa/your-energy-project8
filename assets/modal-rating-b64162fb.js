...
<div class="gif-container"><img src='${h}' alt="${s} gif"/>
    ...
    </li>`},_e=e=>e.results.map(({imgURL:t,name:s,filter:a})=>(t||(t="/images/no-image.png"),`            
      ...
      </li>`)).join(""),xe=(e,t)=>{let s=[];for(let a=1;a<=e.totalPages;a++){let r=a.toString()===e.page.toString()?"current":"";s.push(` <li class="exercises_pagination-item ${r}">
        <a class="page-num" data-page="${a}" data-filter="${t}">${a}</a>
      </li>`)}return s.join("")},p={favorites:"favorites",phrase:"phrase"},I=e=>{const t=d(p.favorites);if(!t){alert("Oops! Refresh page");return}const s=t.filter(({_id:a})=>a!==e);k(p.favorites,s)},H=({name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i})=>{const h=[...d(p.favorites)||[],{name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i}];k("favorites",h)};u.defaults.baseURL="https://your-energy.b.goit.study/api/";const K=async()=>(await u.get("quote")).data,we=async({filter:e="",page:t=1,limit:s=12}={})=>{try{return(await u.get(`filters?filter=${e}&page=${t}&limit=${s}`)).data}catch(a){console.error("Error in getFilters:",a)}},$e=async({bodypart:e="",muscles:t="",equipment:s="",keyword:a="",page:r=1,limit:o=10}={})=>{try{return(await u.get(`exercises?bodypart=${e}&muscles=${t}&equipment=${s}&keyword=${a}&page=${r}&limit=${o}`)).data}catch(i){console.error("Error in getExercises:",i)}},z=async e=>{try{return(await u.get(`exercises/${e}`)).data}catch(t){console.error("Error in getExerciseId:",t)}},V=async(e,t)=>{try{return(await u.patch(`exercises/${e}/rating`,t)).data}catch(s){console.error("Error in updateRaring:",s)}},Le=async e=>{try{return await u.post("subscription",e)}catch(t){return console.error("Error in subscribe:",t),t.response}};function Y(e){document.querySelectorAll("#rating .modal-exercises__rating-star").forEach(s=>{const a=parseInt(s.dataset.value,10);let r=0;a<=e?r=100:a-e<1&&(r=(e-a+1)*100),s.style.background=`linear-gradient(to right, gold ${r}%, gray ${r}%)`,s.style.webkitBackgroundClip="text",s.style.color="transparent"})}function F(){return window.location.href}function G(){const e=window.location.search,t=new URLSearchParams(e),s={};return t.forEach(function(a,r){s[r]=a}),s}function J(e){const t=window.location.search;return new URLSearchParams(t).get(e)}function Q(e,t){const s=new URL(location.href);s.searchParams.set(e,t),history.pushState({},"",s)}function v(e){const t=new URL(location),s=new URLSearchParams(t.search);s.delete(e),t.search=s.toString(),history.pushState(null,"",t)}function ke(){Object.keys(G()).forEach(v)}const y="/your-energy-project8/assets/icons-71caf5e8.svg",X="/your-energy-project8/assets/no-image-7be81c97.png",Z="/your-energy-project8/assets/valkoSprite-28e86a63.svg",P=document.querySelector(".modal-exercises"),ee=window.location.href.split("/").includes("favorites.html");let f;const U=(e=d("favorites")||[])=>e.find(t=>(t==null?void 0:t._id)===f._id),te=e=>{const t=document.querySelector(".modal-exercises__button-favourites");if(!U()){H(e),t.innerHTML=`Unfavorite
    <svg
          ...
          </svg>`};async function O(e){try{let r=function(o){const i=F(),l=document.createElement("input");l.value=i,document.body.appendChild(l),l.select(),document.execCommand("copy"),document.body.removeChild(l),a.textContent="Link copy!",setTimeout(()=>{a.textContent="Share exercises"},1e3)};f=await z(e),P.innerHTML=se(f),Y(f.rating),ae(),document.querySelector(".modal-exercises__card").addEventListener("click",o=>{if(o.target.closest(".modal-exercises__button-favourites")&&te(f),o.target.closest(".modal-exercises__button-rating")){const i=document.querySelector("#ratingModal");i.style.display="block",P.classList.add("visually-hidden")}});const s=document.querySelector(".share-button-modal"),a=document.querySelector(".share-button-hint-modal");s.addEventListener("click",r)}catch(t){console.error(t)}}function se(e){const{name:t,burnedCalories:s,bodyPart:a,description:r,target:o,equipment:i,gifUrl:l,popularity:h,rating:x,time:W,_id:A}=e;return`<div class="modal-exercises__card">
  ...
  </div>`}const n={openModalButton:document.querySelector(".open-modal"),modalExercises:document.querySelector(".modal-exercises"),body:document.body},B=document.getElementById("exercises-list-container");B&&B.addEventListener("click",j);function j(e){if(e.target.matches(".changable-icon-use")||!e.target.closest("[data-id]"))return;const t=e.target.closest("[data-id]").dataset.id;Q("modalOpen",t),O(t)}function ae(){n.modalExercises.addEventListener("click",t),document.addEventListener("keydown",s),e();function e(){n.modalExercises.classList.remove("visually-hidden"),n.body.classList.add("modal-open")}function t(r){r.target.closest(".modal-exercises__button-close")||r.target===n.modalExercises?(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),v("modalOpen")):r.target===n.modalExercises&&(n.modalExercises.classList.add("visually-hidden"),v("modalOpen"))}function s(r){r.key==="Escape"&&(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),v("modalOpen"))}function a(){n.openModalButton.removeEventListener("click",e),n.modalExercises.removeEventListener("click",t),document.removeEventListener("keydown",s)}return a}function re(){const e=J("modalOpen");e&&O(e)}re();const c=document.getElementById("favorite-cards-list"),_=document.getElementById("pagination-container");let S=1,L=window.innerWidth<768?8:10;const oe=({target:e})=>{if(!e.closest("#remove-favorite-btn"))return;const t=e.closest("[data-id]").dataset.id;I(t),E(c)},ie=(e,t=1)=>{if(window.innerWidth<1440){const s=(t-1)*L,a=s+L;return e.slice(s,a).map(r=>M(r,!0)).join("")}return e.map(s=>M(s,!0)).join("")},w=(e="")=>e.length>4&&window.innerWidth<1440?`${e.slice(0,4)} ...`:e,ne=(e="")=>{const t=e.split(" ");return e!==""&&t.length>3?t.splice(0,3).join(" ")+" ...":e};function E(e){const t=document.querySelector(".scrollbar-container"),s=document.querySelector(".text-nocard-container");try{const a=d("favorites");if(!a){t==null||t.classList.add("hidden"),s==null||s.classList.remove("hidden");return}s==null||s.classList.add("hidden"),t==null||t.classList.remove("hidden"),e&&(e.innerHTML=ie(a,S)),window.innerWidth<1440&&ce(a.length)}catch(a){console.log(a.message)}}const ce=e=>{const t=Math.ceil(e/L);let s="";if(d("favorites").length>10||d("favorites").length>8&&window.innerWidth<768)for(let a=1;a<=t;a++)s+=`<li class="pagination-item ${a===S?"current":""}">
                      <a href="#" class="page-number" data-page="${a}">${a}</a>
                    </li>`;_&&(_.innerHTML=s)};_&&_.addEventListener("click",e=>{e.target.classList.contains("page-number")&&(e.preventDefault(),S=parseInt(e.target.dataset.page,10),E(c)),c&&(c.scrollTop=0,c.scrollIntoView({block:"start",behavior:"smooth"}))});E(c);c==null||c.addEventListener("click",j);c==null||c.addEventListener("click",oe);const R=document.getElementById("myModal"),le=document.getElementById("openModalBtn"),de=document.getElementById("closeModalBtn"),T=document.querySelector("body"),m=document.querySelector(".nav_link_favorites"),g=document.querySelector(".nav_link_home");le.addEventListener("click",function(){R.style.display="block",T.classList.add("modal-open")});de.addEventListener("click",function(){R.style.display="none",T.classList.remove("modal-open")});F().includes("favorites")?(g==null||g.classList.remove("nav_item_active"),m==null||m.classList.add("nav_item_active")):(g==null||g.classList.add("nav_item_active"),m==null||m.classList.remove("nav_item_active"));const $="/your-energy-project8/assets/symbol-defs-c6bad215.svg",C="/your-energy-project8/assets/quote-home-desk@1x-acbacd51.jpg",ue="/your-energy-project8/assets/quote-home-desk@2x-d70947ab.jpg",me=document.querySelector(".quote");function ge(e){return`<div class="quote-container container">
...
//# sourceMappingURL=modal-rating-b64162fb.js.map
