import{a as u,i as E}from"./vendor-916d32b4.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function s(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerpolicy&&(o.referrerPolicy=r.referrerpolicy),r.crossorigin==="use-credentials"?o.credentials="include":r.crossorigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=s(r);fetch(r.href,o)}})();const L=(e,t)=>{try{localStorage.setItem(e,JSON.stringify(t))}catch(s){console.error("Error save localStorage:",s)}},d=e=>{try{const t=localStorage.getItem(e);return t?JSON.parse(t):null}catch(t){return console.error("Error get localStorage:",t),null}},D=(e,t)=>e?'<button type="button" class="remove-btn" id="remove-favorite-btn"><svg class="changeble-icon" width="16" height="16"><use href="./oleksii-symbol-defs.svg#icon-trash" class="changable-icon-use"></use></svg></button>':`<div class="icon-rating-container"><p class="rating-info-card">${t}</p><svg class="changeble-icon" width="18" height="18"><use href="./oleksii-symbol-defs.svg#icon-star"></use></svg></div>`,S=(e,t=!1)=>{const{name:s,burnedCalories:a,bodyPart:r,target:o,_id:i,time:l,gifUrl:h,rating:b}=e;return`<li class="favorite-info-card" data-id=${i}>
...
<div class="gif-container"><img src='${h}' alt="${s} gif"/>
      ...
      <ul class="card-info-list">
        <li>
          <p class="item-text">
            Burned kcal: <em class="hidden-overflow-text">${x(`${a}/${l}`)}<strong>&nbspmin</strong></em>
          </p>
        </li>
        <li>
      ...
      </li>`)}return s.join("")},p={favorites:"favorites",phrase:"phrase"},P=e=>{const t=d(p.favorites);if(!t){alert("Oops! Refresh page");return}const s=t.filter(({_id:a})=>a!==e);L(p.favorites,s)},A=({name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i})=>{const h=[...d(p.favorites)||[],{name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i}];L("favorites",h)};u.defaults.baseURL="https://your-energy.b.goit.study/api/";const N=async()=>(await u.get("quote")).data,ve=async({filter:e="",page:t=1,limit:s=12}={})=>{try{return(await u.get(`filters?filter=${e}&page=${t}&limit=${s}`)).data}catch(a){console.error("Error in getFilters:",a)}},fe=async({bodypart:e="",muscles:t="",equipment:s="",keyword:a="",page:r=1,limit:o=10}={})=>{try{return(await u.get(`exercises?bodypart=${e}&muscles=${t}&equipment=${s}&keyword=${a}&page=${r}&limit=${o}`)).data}catch(i){console.error("Error in getExercises:",i)}},W=async e=>{try{return(await u.get(`exercises/${e}`)).data}catch(t){console.error("Error in getExerciseId:",t)}},H=async(e,t)=>{try{return(await u.patch(`exercises/${e}/rating`,t)).data}catch(s){console.error("Error in updateRaring:",s)}},ye=async e=>{try{return await u.post("subscription",e)}catch(t){return console.error("Error in subscribe:",t),t.response}};function K(e){document.querySelectorAll("#rating .modal-exercises__rating-star").forEach(s=>{const a=parseInt(s.dataset.value,10);let r=0;a<=e?r=100:a-e<1&&(r=(e-a+1)*100),s.style.background=`linear-gradient(to right, gold ${r}%, gray ${r}%)`,s.style.webkitBackgroundClip="text",s.style.color="transparent"})}function B(){return window.location.href}function z(){const e=window.location.search,t=new URLSearchParams(e),s={};return t.forEach(function(a,r){s[r]=a}),s}function V(e){const t=window.location.search;return new URLSearchParams(t).get(e)}function Y(e,t){const s=new URL(location.href);s.searchParams.set(e,t),history.pushState({},"",s)}function f(e){const t=new URL(location),s=new URLSearchParams(t.search);s.delete(e),t.search=s.toString(),history.pushState(null,"",t)}function _e(){Object.keys(z()).forEach(f)}const q=document.querySelector(".modal-exercises"),G=window.location.href.split("/").includes("favorites.html");let v;const C=(e=d("favorites")||[])=>e.find(t=>(t==null?void 0:t._id)===v._id),J=e=>{const t=document.querySelector(".modal-exercises__button-favourites");if(!C()){A(e),t.innerHTML=`Unfavorite
    <svg
          ...
          </svg>`;return}P(e._id),t.innerHTML=`Add to favorites
          <svg
          ...
          </svg>`};async function I(e){try{let r=function(o){const i=B(),l=document.createElement("input");l.value=i,document.body.appendChild(l),l.select(),document.execCommand("copy"),document.body.removeChild(l),a.textContent="Link copy!",setTimeout(()=>{a.textContent="Share exercises"},1e3)};v=await W(e),q.innerHTML=Q(v),K(v.rating),X(),document.querySelector(".modal-exercises__card").addEventListener("click",o=>{if(o.target.closest(".modal-exercises__button-favourites")&&J(v),o.target.closest(".modal-exercises__button-rating")){const i=document.querySelector("#ratingModal");i.style.display="block",q.classList.add("visually-hidden")}});const s=document.querySelector(".share-button-modal"),a=document.querySelector(".share-button-hint-modal");s.addEventListener("click",r)}catch(t){console.error(t)}}function Q(e){const{name:t,burnedCalories:s,bodyPart:a,description:r,target:o,equipment:i,gifUrl:l,popularity:h,rating:b,time:U,_id:j}=e;return`<div class="modal-exercises__card">
    ...
    <div class="modal-exercises__image-wrapper">
      <img class="modal-exercises__image" src="${l!==null?l:"./images/no-image.png"}" alt="${t}" />
    </div>
    <div class="modal-exercises__description">
      ...
      <div class="modal-exercises__partials">
        ...
        <div class="modal-exercises__partials-item">
          ...
          <p class="modal-exercises__partials-value">${s}/${U} 
      ...
      ${G||C()?`<button 
        ...
        <button class="modal-exercises__button-rating" data-value="${j}">Give a rating</button>
      </div>
    </div>
  </div>`}const n={openModalButton:document.querySelector(".open-modal"),modalExercises:document.querySelector(".modal-exercises"),body:document.body},M=document.getElementById("exercises-list-container");M&&M.addEventListener("click",F);function F(e){if(e.target.matches(".changable-icon-use")||!e.target.closest("[data-id]"))return;const t=e.target.closest("[data-id]").dataset.id;Y("modalOpen",t),I(t)}function X(){n.modalExercises.addEventListener("click",t),document.addEventListener("keydown",s),e();function e(){n.modalExercises.classList.remove("visually-hidden"),n.body.classList.add("modal-open")}function t(r){r.target.closest(".modal-exercises__button-close")?(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open")):r.target===n.modalExercises?(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),f("modalOpen")):r.target===n.modalExercises&&(n.modalExercises.classList.add("visually-hidden"),f("modalOpen"))}function s(r){r.key==="Escape"&&(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),f("modalOpen"))}function a(){n.openModalButton.removeEventListener("click",e),n.modalExercises.removeEventListener("click",t),document.removeEventListener("keydown",s)}return a}function Z(){const e=V("modalOpen");e&&I(e)}Z();const c=document.getElementById("favorite-cards-list"),_=document.getElementById("pagination-container");let k=1,w=window.innerWidth<768?8:10;const ee=({target:e})=>{if(!e.closest("#remove-favorite-btn"))return;const t=e.closest("[data-id]").dataset.id;P(t),$(c)},te=(e,t=1)=>{if(window.innerWidth<1440){const s=(t-1)*w,a=s+w;return e.slice(s,a).map(r=>S(r,!0)).join("")}return e.map(s=>S(s,!0)).join("")},x=(e="")=>e.length>4&&window.innerWidth<1440?`${e.slice(0,4)} ...`:e,se=(e="")=>{const t=e.split(" ");return e!==""&&t.length>3?t.splice(0,3).join(" ")+" ...":e};function $(e){const t=document.querySelector(".scrollbar-container"),s=document.querySelector(".text-nocard-container");try{const a=d("favorites");if(!a){t==null||t.classList.add("hidden"),s==null||s.classList.remove("hidden");return}s==null||s.classList.add("hidden"),t==null||t.classList.remove("hidden"),e&&(e.innerHTML=te(a,k)),window.innerWidth<1440&&ae(a.length),c&&(c.scrollTop=0,c.scrollIntoView({block:"start",behavior:"smooth"}))}catch(a){console.log(a.message)}}const ae=e=>{const t=Math.ceil(e/w);let s="";if(d("favorites").length>10||d("favorites").length>8&&window.innerWidth<768)for(let a=1;a<=t;a++)s+=`<li class="pagination-item ${a===k?"current":""}">
                      <a href="#" class="page-number" data-page="${a}">${a}</a>
                    </li>`;_&&(_.innerHTML=s)};_&&_.addEventListener("click",e=>{e.target.classList.contains("page-number")&&(e.preventDefault(),k=parseInt(e.target.dataset.page,10),$(c))});$(c);c==null||c.addEventListener("click",F);c==null||c.addEventListener("click",ee);const O=document.getElementById("myModal"),re=document.getElementById("openModalBtn"),oe=document.getElementById("closeModalBtn"),R=document.querySelector("body"),m=document.querySelector(".nav_link_favorites"),g=document.querySelector(".nav_link_home");re.addEventListener("click",function(){O.style.display="block",R.classList.add("modal-open")});oe.addEventListener("click",function(){O.style.display="none",R.classList.remove("modal-open")});B().includes("favorites")?(g==null||g.classList.remove("nav_item_active"),m==null||m.classList.add("nav_item_active")):(g==null||g.classList.add("nav_item_active"),m==null||m.classList.remove("nav_item_active"));const ie=document.querySelector(".quote");function ne(e){return`<div class="quote-container container">
  ...
  </div>`}const y=new Date;function ce(e){return e?y.getDate()===e.getDate()&&y.getMonth()===e.getMonth()&&y.getFullYear()===e.getFullYear():!1}async function le(){const e=d(p.phrase);if(!e||!ce(new Date(e.date))){const s=await N();L(p.phrase,{...s,date:y})}const t=d(p.phrase);ie.innerHTML=ne(t)}le();document.querySelector(".js-loader");const T=document.querySelector("#ratingForm"),de=document.querySelector(".close-rating"),ue=document.querySelector("#ratingModal");async function me(e){e.preventDefault();const s=document.querySelector(".modal-exercises__button-rating").getAttribute("data-value"),a=new FormData(T),r={};a.forEach((o,i)=>{i==="rate"?r[i]=Number(o):r[i]=o});try{await H(s,r)?E.success({title:"OK",message:"Successfully sent rating!"}):E.error({title:"Error",message:"There was an error"})}catch{}}T.addEventListener("submit",me);de.addEventListener("click",()=>{const e=document.querySelector(".modal-exercises");document.querySelector(".modal-exercises__card"),e.classList.remove("visually-hidden"),ue.style.display="none"});export{V as a,ve as b,pe as c,he as d,d as e,fe as f,z as g,S as h,B as i,ye as j,_e as r,Y as s};
//# sourceMappingURL=modal-rating-9ee7bb28.js.map
