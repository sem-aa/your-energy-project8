...
<div class="gif-container"><img src='${v}' alt="${s} gif"/>
      ...
      </li>`)}return s.join("")},p={favorites:"favorites",phrase:"phrase"},S=e=>{const t=l(p.favorites);if(!t){alert("Oops! Refresh page");return}const s=t.filter(({_id:a})=>a!==e);k(p.favorites,s)},T=({name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i})=>{const d=l(p.favorites)||[];console.log(d);const v=[...d,{name:e,burnedCalories:t,bodyPart:s,target:a,_id:r,time:o,gifUrl:i}];k("favorites",v)};u.defaults.baseURL="https://your-energy.b.goit.study/api/";const O=async()=>(await u.get("quote")).data,ge=async({filter:e="",page:t=1,limit:s=12}={})=>{try{return(await u.get(`filters?filter=${e}&page=${t}&limit=${s}`)).data}catch(a){console.error("Error in getFilters:",a)}},me=async({bodypart:e="",muscles:t="",equipment:s="",keyword:a="",page:r=1,limit:o=10}={})=>{try{return(await u.get(`exercises?bodypart=${e}&muscles=${t}&equipment=${s}&keyword=${a}&page=${r}&limit=${o}`)).data}catch(i){console.error("Error in getExercises:",i)}},R=async e=>{try{return(await u.get(`exercises/${e}`)).data}catch(t){console.error("Error in getExerciseId:",t)}},W=async(e,t)=>{try{return(await u.patch(`exercises/${e}/rating`,t)).data}catch(s){console.error("Error in updateRaring:",s)}},pe=async e=>{try{return await u.post("subscription",e)}catch(t){return console.error("Error in subscribe:",t),t.response}};function N(e){document.querySelectorAll("#rating .modal-exercises__rating-star").forEach(s=>{const a=parseInt(s.dataset.value,10);let r=0;a<=e?r=100:a-e<1&&(r=(e-a+1)*100),s.style.background=`linear-gradient(to right, gold ${r}%, gray ${r}%)`,s.style.webkitBackgroundClip="text",s.style.color="transparent"})}const A=document.querySelector(".modal-exercises"),U=window.location.href.split("/").includes("favorites.html");let f;const M=(e=l("favorites")||[])=>e.find(t=>(t==null?void 0:t._id)===f._id),H=e=>{const t=document.querySelector(".modal-exercises__button-favourites");if(console.log(e),!M()){T(e),t.innerHTML=`Unfavorite
    <svg
          ...
          </svg>`};async function K(e){try{f=await R(e),A.innerHTML=Y(f),N(f.rating),z(),document.querySelector(".modal-exercises__card").addEventListener("click",s=>{s.target.closest(".modal-exercises__button-favourites")&&(console.log("Button clicked!"),H(f))})}catch(t){console.error(t)}}function Y(e){const{name:t,burnedCalories:s,bodyPart:a,description:r,target:o,equipment:i,gifUrl:d,popularity:v,rating:b,time:j,_id:ce}=e;return`<div class="modal-exercises__card">
  ...
  </div>`}const h=new Date;function ae(e){return console.log("checkedDate:",e),e?(console.log("123",h.getDate()),h.getDate()===e.getDate()&&h.getMonth()===e.getMonth()&&h.getFullYear()===e.getFullYear()):!1}async function re(){const e=l(p.phrase);if(!e||!ae(new Date(e.date))){const s=await O();k(p.phrase,{...s,date:h})}const t=l(p.phrase);te.innerHTML=se(t)}re();document.querySelector(".js-loader");const F=document.querySelector("#ratingForm"),oe=document.querySelector(".close-rating"),y=document.querySelector(".modal-exercises__button-rating"),P=document.querySelector("#ratingModal");console.log(y);const ie="64f389465ae26083f39b17a4";async function ne(e){e.preventDefault();const t=new FormData(F),s={};t.forEach((a,r)=>{r==="rate"?s[r]=Number(a):s[r]=a});try{const a=await W(ie,s);console.log(a),a||alert("Error")}catch(a){console.log("shpm"),console.log(a.message)}}F.addEventListener("submit",ne);y==null||y.addEventListener("click",function(){P.style.display="block",body.classList.add("modal-open")});oe.addEventListener("click",()=>{P.style.display="none"});export{ue as a,l as b,de as c,me as d,L as e,X as f,ge as g,pe as s};
//# sourceMappingURL=modal-rating-d5f8ccf4.js.map
