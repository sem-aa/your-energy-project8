import{b as u,i as W}from"./vendor-eee4493b.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function s(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=s(r);fetch(r.href,o)}})();const k=(e,t)=>{try{localStorage.setItem(e,JSON.stringify(t))}catch(s){console.error("Error save localStorage:",s)}},d=e=>{try{const t=localStorage.getItem(e);return t?JSON.parse(t):null}catch(t){return console.error("Error get localStorage:",t),null}},O=window.location.href.split("/").includes("favorites.html"),z=e=>e?"favorite-info-card width-for-fav-card":"favorite-info-card",D=(e,t)=>e?'<button type="button" class="remove-btn" data-remove aria-label="Remove"><svg class="changeble-icon" width="16" height="16"><use href="./oleksii-symbol-defs.svg#icon-trash" class="changable-icon-use"></use></svg></button>':`<div class="icon-rating-container"><p class="rating-info-card">${t}</p><svg class="changeble-icon" width="18" height="18"><use href="./oleksii-symbol-defs.svg#icon-star"></use></svg></div>`,P=(e,t=!1)=>{const{name:s,burnedCalories:a,bodyPart:r,target:o,_id:i,time:c,gifUrl:f,rating:_}=e;return`<li class='${z(O)}' data-id=${i}>
...
<div class="gif-container"><img src='${f}' alt="${s} gif"/>
    ...
    </li>`},We=e=>e.results.map(({imgURL:t,name:s,filter:a})=>(t||(t="/images/no-image.png"),`            
    ...
    <svg
          ...
          </svg>`,b(B);return}C(e._id),t.innerHTML=`Add to favorites
          <svg
          ...
          </svg>`,b(B)};async function Y(e){try{let r=function(o){const i=I(),c=document.createElement("input");c.value=i,document.body.appendChild(c),c.select(),document.execCommand("copy"),document.body.removeChild(c),a.textContent="Link copy!",setTimeout(()=>{a.textContent="Share exercises"},1e3)};m=await T(e),console.log(Number(m.rating).toFixed(1)),U.innerHTML=ne({...m,rating:Number(m.rating).toFixed(1)}),Z(m.rating),ce(),document.querySelector(".modal-exercises__card").addEventListener("click",o=>{if(o.target.closest(".modal-exercises__button-favourites")&&ie(m),o.target.closest(".modal-exercises__button-rating")){const i=document.querySelector("#ratingModal");i.style.display="block",U.classList.add("visually-hidden")}});const s=document.querySelector(".share-button-modal"),a=document.querySelector(".share-button-hint-modal");s.addEventListener("click",r)}catch(t){console.error(t)}}function ne(e){const{name:t,burnedCalories:s,bodyPart:a,description:r,target:o,equipment:i,gifUrl:c,popularity:f,rating:_,time:K,_id:H}=e;return`<div class="modal-exercises__card">
  ...
  </div>`}const n={openModalButton:document.querySelector(".open-modal"),modalExercises:document.querySelector(".modal-exercises"),body:document.body},M=document.getElementById("exercises-list-container");M&&M.addEventListener("click",V);function V(e){if(e.target.closest("[data-remove]")||!e.target.closest("[data-id]"))return;const t=e.target.closest("[data-id]").dataset.id;se("modalOpen",t),Y(t)}function ce(){n.modalExercises.addEventListener("click",t),document.addEventListener("keydown",s),e();function e(){n.modalExercises.classList.remove("visually-hidden"),n.body.classList.add("modal-open")}function t(r){r.target.closest(".modal-exercises__button-close")||r.target===n.modalExercises?(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),h("modalOpen")):r.target===n.modalExercises&&(n.modalExercises.classList.add("visually-hidden"),h("modalOpen"))}function s(r){r.key==="Escape"&&(n.modalExercises.classList.add("visually-hidden"),n.body.classList.remove("modal-open"),h("modalOpen"))}function a(){n.openModalButton.removeEventListener("click",e),n.modalExercises.removeEventListener("click",t),document.removeEventListener("keydown",s)}return a}function le(){const e=te("modalOpen");e&&Y(e)}le();const l=document.getElementById("favorite-cards-list"),S=document.getElementById("pagination-container"),de=document.querySelector(".quote-container");let E=1,A=window.innerWidth<768?8:10;const ue=window.location.href.split("/").includes("favorites.html"),me=({target:e})=>{if(!e.closest("[data-remove]"))return;const t=e.closest("[data-id]").dataset.id;C(t),b(l)};function pe(){let e=window.innerWidth;function t(){let s=window.innerWidth;s!==e&&(window.location.reload(),e=s)}window.addEventListener("resize",t)}const ge=(e,t=1)=>{if(window.innerWidth<1440){const s=(t-1)*A,a=s+A;return e.slice(s,a).map(r=>P(r,!0)).join("")}return e.map(s=>P(s,!0)).join("")},L=(e="")=>e.length>4&&window.innerWidth<1440?`${e.slice(0,4)} ...`:e,ve=(e="")=>{const t=e.split(" ");return e!==""&&t.length>3?t.splice(0,3).join(" ")+" ...":e};function b(e){const t=document.querySelector(".scrollbar-container"),s=document.querySelector(".text-nocard-container");try{const a=d("favorites");if(!a||a.length===0){t==null||t.classList.add("hidden"),s==null||s.classList.remove("hidden");return}s==null||s.classList.add("hidden"),t==null||t.classList.remove("hidden"),e&&(e.innerHTML=ge(a,E)),window.innerWidth<1440&&fe(a.length),ue&&window.innerWidth>=1440&&(de.style.maxWidth="494px")}catch(a){console.log(a.message)}}const fe=e=>{const t=Math.ceil(e/A);let s="";if(d("favorites").length>10||d("favorites").length>8&&window.innerWidth<768)for(let a=1;a<=t;a++)s+=`<li class="pagination-item ${a===E?"current":""}">
  ...
  </div>`}const x=new Date;function _e(e){return e?x.getDate()===e.getDate()&&x.getMonth()===e.getMonth()&&x.getFullYear()===e.getFullYear():!1}async function Le(){const e=d(v.phrase);if(!e||!_e(new Date(e.date))){const s=await J();k(v.phrase,{...s,date:x})}const t=d(v.phrase);w&&(w.innerHTML=we(t),Se())}Le();document.querySelector(".loader");const R=document.querySelector("#ratingForm"),qe=document.querySelector(".close-rating"),Ae=document.querySelector("#ratingModal");async function ke(e){e.preventDefault();const s=document.querySelector(".modal-exercises__button-rating").getAttribute("data-value"),a=new FormData(R),r={};a.forEach((o,i)=>{i==="rate"?r[i]=Number(o):r[i]=o});try{await G(s,r)?(W.success({title:"OK",message:"Successfully sent rating!"}),F()):W.error({title:"Error",message:"There was an error"}),e.target.reset()}catch{}}R.addEventListener("submit",ke);qe.addEventListener("click",F);function F(){document.querySelector(".modal-exercises").classList.remove("visually-hidden"),Ae.style.display="none"}export{Ue as F,Be as I,v as K,ee as a,te as b,Me as c,We as d,Pe as e,Xe as f,d as g,P as h,I as i,Ce as j,Ie as r,se as s};
//# sourceMappingURL=modal-rating-5d35485c.js.map
