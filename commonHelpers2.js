import{c as j,a as B,b as V,g as O,d as W,r as z}from"./assets/header-93f8c60d.js";import{a as l,i as g,S as G}from"./assets/vendor-916d32b4.js";const _=document.getElementById("to-top-btn"),C=document.querySelector(".btn-up");window==null||window.addEventListener("scroll",K);function K(){document.body.scrollTop>30||document.documentElement.scrollTop>30?C.style.display="flex":C.style.display="none"}_==null||_.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});document.querySelector(".js-loader");l.defaults.baseURL="https://your-energy.b.goit.study/api/";const J=async({filter:t="",page:e=1,limit:s=12}={})=>{try{return(await l.get(`filters?filter=${t}&page=${e}&limit=${s}`)).data}catch(r){console.error("Error in getFilters:",r)}},Q=async({bodypart:t="",muscles:e="",equipment:s="",keyword:r="",page:a=1,limit:o=10}={})=>{try{return(await l.get(`exercises?bodypart=${t}&muscles=${e}&equipment=${s}&keyword=${r}&page=${a}&limit=${o}`)).data}catch(n){console.error("Error in getExercises:",n)}},X=async t=>{try{return(await l.get(`exercises/${t}`)).data}catch(e){console.error("Error in getExerciseId:",e)}},Y=async(t,e)=>{try{return(await l.patch(`exercises/${t}/rating`,e)).data}catch(s){console.error("Error in getExerciseId:",s)}},Z=async t=>{try{return await l.post("subscription",t)}catch(e){return console.error("Error in subscribe:",e),e.response}},N=12,x=document.querySelector(".search-box"),b=document.querySelector(".section-title_additional"),p=document.querySelector("button[data-muscles]"),v=document.querySelector("button[data-body]"),y=document.querySelector("button[data-equipment]"),d=document.querySelector("#category-list-container"),L=document.querySelector("#exercises-list-container"),M=document.querySelector(".exercises_pagination"),k=document.querySelector("#exercises");u("Muscles");function u(t,e=1){J({filter:t,page:e,limit:N}).then(s=>{var r,a,o,n;(r=d==null?void 0:d.classList)==null||r.remove("visually-hidden"),(a=L==null?void 0:L.classList)==null||a.add("visually-hidden"),(o=x==null?void 0:x.classList)==null||o.add("visually-hidden-ext"),(n=b==null?void 0:b.classList)==null||n.add("visually-hidden"),s.results.length?(d.innerHTML=j(s),s.totalPages>1?(M.innerHTML=B(s,t),ae()):M.innerHTML=""):d.innerHTML='<li class="sorry-message"><p>Sorry, there are no exercises by your request.</p></li>'})}const ee=()=>{q(p),u("Muscles")},te=()=>{q(v),u("Body parts")},se=()=>{q(y),u("Equipment")};p==null||p.addEventListener("click",ee);v==null||v.addEventListener("click",te);y==null||y.addEventListener("click",se);function q(t){document.querySelector(".active").classList.remove("active"),t.classList.contains("active")?t.classList.remove("active"):t.classList.add("active")}function ae(){document.querySelectorAll("a.page-num").forEach(function(e){e==null||e.addEventListener("click",function(s){k==null||k.scrollIntoView({block:"start",behavior:"smooth"}),u(s.target.dataset.filter,s.target.dataset.page)})})}const F=document.querySelector(".search-box"),f=document.querySelector(".search-input"),re=document.querySelector("#search-button"),oe=document.querySelector(".section-title_additional"),ie=document.querySelector("#title-category"),I=document.querySelector("#category-list-container"),S=document.querySelector("#exercises-list-container"),$=document.querySelector(".exercises_pagination"),ne=document.querySelector("#exercises"),E=10;let i={};F.addEventListener("keydown",t=>{if(t.key==="Enter"){if(H()==!1)return;h(i.filter,i.category,1,f.value)}});re.addEventListener("click",()=>{H()!=!1&&h(i.filter,i.category,1,f.value)});function H(){return f.value.trim()===""?(g.show({title:"Warning",message:"Please enter your search query.",position:"topRight",color:"yellow"}),!1):!0}I.addEventListener("click",ce);async function ce(t){const e=t.target.closest(".exercises_category-item");e&&(!e.dataset.filter||!e.dataset.category||(I.classList.add("visually-hidden"),S.classList.remove("visually-hidden"),F.classList.remove("visually-hidden-ext"),oe.classList.remove("visually-hidden"),f.value="",h(e.dataset.filter,e.dataset.category)))}async function h(t,e,s=1,r=""){let a=r.trim().toLowerCase();switch(t){case"Muscles":i={muscles:e,category:e,filter:t,keyword:a,limit:E,page:s};break;case"Equipment":i={equipment:e,category:e,filter:t,keyword:a,limit:E,page:s};break;case"Body parts":i={bodypart:e,category:e,filter:t,keyword:a,limit:E,page:s};break}ie.innerHTML=e,Q(i).then(o=>{o.results.length?S.innerHTML=o.results.map(n=>V(n)).join(""):S.innerHTML='<li class="sorry-message"><p>Sorry, there are no exercises by your request.</p></li>',o.totalPages>1?($.innerHTML=B(o,t),le()):$.innerHTML=""})}function le(){document.querySelectorAll("a.page-num").forEach(function(e){e.addEventListener("click",function(s){ne.scrollIntoView({block:"start",behavior:"smooth"}),h(s.target.dataset.filter,i.category,s.target.dataset.page,i.keyword)})})}const de=document.querySelector("#sorted-select");new G({select:"#sorted-select",settings:{showSearch:!1},data:[{text:"Default sort",value:"default",class:"sorted-option"},{text:"Best Match",value:"best",class:"sorted-option"},{text:"Calories: lowest first",value:"calories-lowest",class:"sorted-option"},{text:"Calories: highest first",value:"calories-highest",class:"sorted-option"}]});const ue=t=>{const e=t.map(createInfoCardMarkup).join("");favoritesList.innerHTML=e};de.addEventListener("change",me);function me(t){const e=t.target.value,s=ge({sortType:e,array:arrayCardsForTest});ue(s)}function ge({sortType:t,array:e}){let s;switch(t){case"default":s=e.toSorted((r,a)=>r.name.localeCompare(a.name));break;case"best":s=e.toSorted((r,a)=>a.rating-r.rating);break;case"calories-highest":s=e.toSorted((r,a)=>a.burnedCalories-r.burnedCalories);break;case"calories-lowest":s=e.toSorted((r,a)=>r.burnedCalories-a.burnedCalories);break;default:s=e.toSorted((r,a)=>r.name.localeCompare(a.name));break}return s}function pe(){return window.location.href}const ve=document.querySelector(".share-button"),T=document.querySelector(".share-button-hint");ve.addEventListener("click",ye);function ye(t){const e=pe(),s=document.createElement("input");s.value=e,document.body.appendChild(s),s.select(),document.execCommand("copy"),document.body.removeChild(s),T.textContent="Link copy!",setTimeout(()=>{T.textContent="Share exercises"},1e3)}const R=document.querySelector(".footer-form");R.addEventListener("submit",fe);function fe(t){t.preventDefault();const e={email:t.target.elements.email.value};R.reset(),Z(e).then(s=>{switch(s.status){case 201:g.success({message:s.data.message});break;case 409:g.info({message:s.data.message});break;default:g.warning({message:"Sorry, something went wrong"});break}})}const c={openModalButton:document.querySelector(".open-modal"),modalExercises:document.querySelector(".modal-exercises")},he=document.getElementById("exercises-list-container");he.addEventListener("click",_e);function _e(t){if(!t.target.closest("[data-id]"))return;const e=t.target.closest("[data-id]").dataset.id;Ee(e)}function xe(){c.modalExercises.addEventListener("click",e),document.addEventListener("keydown",s),t();function t(){c.modalExercises.classList.remove("visually-hidden")}function e(a){(a.target.closest(".modal-exercises__button-close")||a.target===c.modalExercises)&&c.modalExercises.classList.add("visually-hidden")}function s(a){a.key==="Escape"&&c.modalExercises.classList.add("visually-hidden")}function r(){c.openModalButton.removeEventListener("click",t),c.modalExercises.removeEventListener("click",e),document.removeEventListener("keydown",s)}return r}function be(t){document.querySelectorAll("#rating .modal-exercises__rating-star").forEach(s=>{const r=parseInt(s.dataset.value,10);let a=0;r<=t?a=100:r-t<1&&(a=(t-r+1)*100),s.style.background=`linear-gradient(to right, gold ${a}%, gray ${a}%)`,s.style.webkitBackgroundClip="text",s.style.color="transparent"})}const Le=document.querySelector(".modal-exercises");let m;const ke=t=>{const e=document.querySelector(".modal-exercises__button-favourites");if(console.log(t),!(O("favorites")||[]).find(a=>(a==null?void 0:a._id)===t._id)){W(t),e.innerHTML=`Unfavorite
    <svg
          ...
          </svg>`;return}z(t._id),e.innerHTML=`Add to favorites
          <svg
            class="modal-exercises__button-favourites_icon"
            aria-label='heart'
            width="20"
          ...
          </svg>`};async function Ee(t){try{m=await X(t),Le.innerHTML=Se(m),be(m.rating),xe(),console.log(123),document.querySelector(".modal-exercises__card").addEventListener("click",s=>{s.target.closest(".modal-exercises__button-favourites")&&(console.log("Button clicked!"),ke(m))})}catch(e){console.error(e)}}function Se(t){const{name:e,burnedCalories:s,bodyPart:r,description:a,target:o,equipment:n,gifUrl:w,popularity:P,rating:U,time:D,_id:$e}=t;return`<div class="modal-exercises__card">
    ...
    <div class="modal-exercises__description">
      ...
      <div class="modal-exercises__partials">
        ...
        <div class="modal-exercises__partials-item">
          ...
          <p class="modal-exercises__partials-value">${s}/${D} 
  ...
  </div>`}const A=document.querySelector("#ratingForm"),qe=document.querySelector(".close-rating"),we=document.querySelector("#ratingModal"),Ce="64f389465ae26083f39b17a4";function Me(t){t.preventDefault();const e=new FormData(A),s={};e.forEach((r,a)=>{s[a]=r}),console.log(s),Y(Ce,s)}A.addEventListener("submit",Me);qe.addEventListener("click",()=>{we.style.display="none"});
